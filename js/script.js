...
(function () {
	...
	$(function () {
		...
		function attachFormValidator(elements) {
			...
			var regularConstraintsMessages = [
				...
				},
				{
					...
					newMessage: "Das ist keine E-Mailadresse"
				},
				{
					type: regula.Constraint.Numeric,
					newMessage: "Nur Nummern möglich"
				}
			];
		...
		if (plugins.rdMailForm.length) {
			var i, j, k,
				msg = {
					'MF255': 'Ihre Meldung wurde erfolgreich abgeschickt. Wir melden uns!'
				};
		...
		$("#contact-form").on("submit", function (e) {
			...
			if (!isValidated($form.find('[data-constraints]'))) return;
			e.preventDefault;

			var data = $form.serializeArray();

			// save data to localstorage as string
			localStorage.setItem('contactData', JSON.stringify(data));

			// hide contact
			$form.attr('hidden', true)
			$('#contact-placeholder').removeAttr('hidden')

			data.forEach(function (entry) {
				$('#contact-' + entry.name + '-placeholder').text(entry.value);
			})
		})
	});
}());
